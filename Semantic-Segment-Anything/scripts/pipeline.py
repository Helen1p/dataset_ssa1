...
from generate_region import region_regress, add_region_distortion
import cv2
...
def semantic_annotation_pipeline(filename, data_path, output_path_json, output_path_semantic, output_path_dis, rank, save_img=False, scale_small=1.2, scale_large=1.6, scale_huge=1.6,
    ...
    h, w, _ = img.shape
    # 太多了，去掉一些
    anns = region_regress(anns, 16, img_area=h*w)
    ...
    if len(anns['annotations']) ==0:
        ...
        print('[Save] save SSA-engine annotation results: ', os.path.join(output_path_json, filename + '_semantic.json'))
        if save_img:
